...
| **Postcondiciones** | El proceso de onboarding cambia a un estado final (`Validado`) o se devuelve al flujo de corrección (`Rechazado_Subsanacion`) registrando el histórico de validaciones. |
...
stateDiagram-v2
    ...
    Creado --> Enviado_Enlace : Sistema envía email con token OTP
    Enviado_Enlace --> En_Progreso : Empleado accede con enlace válido
    Enviado_Enlace --> Enviado_Enlace : Admin reenvía enlace (regenera token)
    En_Progreso --> Completado_Pendiente_Validacion : Empleado envía formulario completo
    Completado_Pendiente_Validacion --> Validado : Admin aprueba toda la información
    Completado_Pendiente_Validacion --> Rechazado_Subsanacion : Admin rechaza >= 1 campo o archivo
    Rechazado_Subsanacion --> Completado_Pendiente_Validacion : Empleado corrige y reenvía
